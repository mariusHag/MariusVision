...
document.addEventListener('scroll2', () => {
    ...
    paralexImages.forEach(image => {
        ...
        const scrollY = window.scrollY; // Global scroll position

        // Calculate relative scroll effect
        const newY = (scrollY - rect.top) * speed;
        
        image.style.transform = `translate3d(0, ${newY}px, 0)`; // GPU acceleration
    });
});
